...
class Game {
  constructor(canvas, snippet) {
    ...
    this.code = __WEBPACK_IMPORTED_MODULE_0__code_snippets__["a" /* samples */][Math.floor(Math.random() * __WEBPACK_IMPORTED_MODULE_0__code_snippets__["a" /* samples */].length)];
    this.posX = 0;
    this.posY = 510;
    this.compX = 890;
    this.compY = 510;
    ...
    this.totalErrors = 0;
    this.typedKeys = 0;
    this.typeableKeys = 50;
    this.numErrors = 0;
    this.chars = null;
    this.startTime = null;
    this.increment = 500 / this.typeableKeys;
    this.gameOver = false;
  ...
  endGame(endTime) {
    ...
    div.innerHTML = [
      ...
      `Accuracy (initial errors only): ${
        Math.abs((this.totalErrors / this.typedKeys) * 100 - 100)
          .toFixed(2)}%`
    ].join('\n\n');
  ...
  handleRace(e) {
    ...
    this.typedKeys += 1;
    console.log([e.key, this.chars[this.target].innerHTML]);
    if (this.numErrors === 0 && (e.key === this.chars[this.target].textContent) ||
        (e.key === 'Enter' && this.chars[this.target].textContent.length > 1)) {
      this.advanceCursor();
  ...
  advanceCursor() {
    if (this.chars[this.target].textContent.length > 1) {
...
["receiveRootNodeIDEvent: function(",
"  rootNodeID: number,",
") {",
...
// export const CODE = samples[Math.floor(Math.random() * samplses.length)];
